...
from utils.ui_helpers import (
    ...
    show_success, show_loading, format_date, show_toast
)
...
def main():
    ...
    # Tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📋 Profile Info", "📚 Reading History", "📊 Analytics", "🔐 Account"])
    ...
    with tab1:
        ...
        with col3:
            st.metric(
                "Streak",
                f"{profile.get('reading_streak', 0)} days",
                "🔥"
            )
                ...
                for idx, item in enumerate(history[:10]):
                    with st.container():
                        ...
                        with col1:
                            ...
                            st.caption(f"📰 {item.get('source', 'Unknown')} • {format_date(item.get('read_at', ''))}")
